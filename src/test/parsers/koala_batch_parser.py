...
from test.schema_packages.koala_package import (
    koala_Batch,
    koala_Cleaning,
    koala_Evaporation,
    koala_Inkjet_Printing,
    koala_LaserScribing,
    koala_Process,
    koala_Sample,
    koala_SlotDieCoating,
    koala_SpinCoating,
    koala_Sputtering,
    koala_Substrate,
    koala_AtomicLayerDeposition,
    ProcessParameter,
)
...
class RawkoalaExperiment(EntryData):
...
class koalaExperimentParser(MatchingParser):
    def is_mainfile(
        self,
        filename: str,
        mime: str,
    ...
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        archives = [map_batch(sample_ids, batch_id, upload_id, koala_Batch)]
        substrates = []
        substrates_col = [
            'Sample dimension',
            'Sample area [cm^2]',
        ...
        for i, sub in df['Experiment Info'][substrates_col].drop_duplicates().iterrows():
            if pd.isna(sub).all():
                continue
            substrates.append((f'{i}_substrate', sub, map_substrate(sub, koala_Substrate)))
        ...
        for i, row in df['Experiment Info'].iterrows():
            ...
            archives.append(map_basic_sample(row, substrate_name, upload_id, koala_Sample))
        ...
        for i, col in enumerate(df.columns.get_level_values(0).unique()):
            ...
            for j, row in df_dropped.iterrows():
                ...
                if 'Cleaning' in col:
                    archives.append(map_cleaning(i, j, lab_ids, row, upload_id, koala_Cleaning))

                if 'Laser Scribing' in col:
                    archives.append(map_laser_scribing(i, j, lab_ids, row, upload_id, koala_LaserScribing))

                if 'Generic Process' in col:  # move up
                    generic_process = map_generic(i, j, lab_ids, row, upload_id, koala_Process)
                ...
                if 'Evaporation' in col:
                    ...
                    if 'Co-Evaporation' in col:
                        coevap = True
                    archives.append(
                        map_evaporation(i, j, lab_ids, row, upload_id, koala_Evaporation, coevap)
                    )

                if 'Spin Coating' in col:
                    archives.append(map_spin_coating(i, j, lab_ids, row, upload_id, koala_SpinCoating))

                if 'Slot Die Coating' in col:
                    archives.append(map_sdc(i, j, lab_ids, row, upload_id, koala_SlotDieCoating))

                if 'Sputtering' in col:
                    archives.append(map_sputtering(i, j, lab_ids, row, upload_id, koala_Sputtering))

                if 'Inkjet Printing' in col:
                    archives.append(
                        map_inkjet_printing(i, j, lab_ids, row, upload_id, koala_Inkjet_Printing)
                    )

                if 'ALD' in col:
                    archives.append(
                        map_atomic_layer_deposition(
                            i, j, lab_ids, row, upload_id, koala_AtomicLayerDeposition)
                    )
        ...
        archive.data = RawkoalaExperiment(processed_archive=refs)
