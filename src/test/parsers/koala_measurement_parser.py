...
from test.schema_packages.koala_package import (
    koala_EQEmeasurement,
    koala_JVmeasurement,
    koala_Measurement,
    koala_SimpleMPPTracking,
)
...
class RawFilekoala(EntryData):
    processed_archive = Quantity(
        type=Activity,
    )


class koalaParser(MatchingParser):
    def parse(self, mainfile: str, archive: EntryArchive, logger):
        ...
        entry = koala_Measurement()

        if measurment_type == 'jv':
            entry = koala_JVmeasurement()
        if measurment_type == 'eqe':
            entry = koala_EQEmeasurement()
        if measurment_type == 'mppt':
            entry = koala_SimpleMPPTracking()
        ...
        archive.data = RawFilekoala(processed_archive=get_reference(archive.metadata.upload_id, eid))
